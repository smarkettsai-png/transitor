...
fn build_window(label: &str, title: &str) -> (Window, bool) {
    ...
    match app_handle.get_window(label) {
        ...
        None => {
            ...
            {
                // Recent system WebView2 runtimes can render Tauri transparent
                // windows as an entirely transparent surface. Keep the fixed
                // runtime's existing appearance, but use an opaque surface for
                // system WebView2 so the translation UI remains visible.
                let fixed_runtime_present = std::env::current_exe()
                    .ok()
                    ...
                    .unwrap_or(false);
                info!(
                    "Windows WebView2 transparent window mode: {}",
                    fixed_runtime_present
                );
                builder = builder
                    .transparent(fixed_runtime_present)
                    .decorations(false);
            }
            #[cfg(target_os = "linux")]
            {
...
fn reveal_translate_window(window: &Window) {
    let hide_window = get("translate_hide_window")
    ...
    if let Err(error) = window.show() {
        warn!("Failed to show translate window: {}", error);
    }
    if let Err(error) = window.set_focus() {
        warn!("Failed to focus translate window: {}", error);
    }
}
...
fn translate_window() -> Window {
    ...
    if exists {
        reveal_translate_window(&window);
        return window;
    }
...
pub fn selection_translate() {
    ...
    // A global shortcut can also be pressed while the translation window owns
    // focus. In that case the `selection` crate asks Windows for the
    // foreground selection and may receive an empty result from WebView2.
    // Let the frontend read its own textarea/DOM selection instead.
    if let Some(window) = app_handle.get_window("translate") {
        if window.is_focused().unwrap_or(false) {
            if let Err(error) = window.emit("selection_translate", ()) {
                warn!("Failed to emit in-window selection event: {}", error);
            }
            return;
        }
    }
...
pub fn ocr_recognize() {
    #[cfg(target_os = "macos")]
    {
        ...
        let mut app_cache_dir_path =
            portable::cache_dir(&app_handle.config().tauri.bundle.identifier)
