...
import { getSpeechLanguage, getSpeechText } from '../../../../utils/translate_result';
...
export default function TargetArea(props) {
    ...
    const resultSpeechText = getSpeechText(result);
    ...
    useEffect(() => {
        ...
        clipboardMonitor,
        sourceLanguage === 'auto' ? detectLanguage : '',
    ]);
    ...
    const addToHistory = async (text, source, target, serviceInstanceKey, result) => {
        try {
            ...
            // History must never prevent a translation result from rendering.
            logError(`[history] failed to save result: ${historyError}`);
        }
    };
    ...
    useEffect(() => {
        if (ttsServiceList && getServiceSouceType(ttsServiceList[0]) === ServiceSourceType.PLUGIN) {
            readTextFile(configPath(`plugins/tts/${getServiceName(ttsServiceList[0])}/info.json`)).then((infoStr) => {
    ...
    const handleSpeak = async () => {
        ...
        if (resultSpeechText === '') {
            throw new Error('No speakable translation result');
        }
        const speechLanguage = getSpeechLanguage(result, {
            sourceLanguage,
            detectLanguage,
            targetLanguage,
        });
        ...
        if (getServiceSouceType(instanceKey) === ServiceSourceType.PLUGIN) {
            if (!ttsPluginInfo?.language) {
                throw new Error('TTS plugin is still loading');
            }
            const pluginConfig = serviceInstanceConfigMap[instanceKey];
            if (!(speechLanguage in ttsPluginInfo.language)) {
                throw new Error('Language not supported');
            }
            let [func, utils] = await invoke_plugin('tts', getServiceName(instanceKey));
            let data = await func(resultSpeechText, ttsPluginInfo.language[speechLanguage], {
                config: pluginConfig,
                utils,
            });
            await speak(data);
        } else {
            const ttsService = builtinTtsServices[getServiceName(instanceKey)];
            if (!ttsService || !(speechLanguage in ttsService.Language)) {
                throw new Error('Language not supported');
            }
            const instanceConfig = serviceInstanceConfigMap[instanceKey];
            let data = await ttsService.tts(resultSpeechText, ttsService.Language[speechLanguage], {
                config: instanceConfig,
            });
            await speak(data);
        }
