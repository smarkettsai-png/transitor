...
import { getSelectedText } from '../../../../utils/selection_text';
...
export default function SourceArea(props) {
    ...
    // When the global selection shortcut is pressed while this window owns
    // focus, read the selection from WebView2 instead of asking Windows for
    // the foreground application's selection. This also covers selections in
    // the source textarea and rendered result cards.
    useEffect(() => {
        if (hideWindow === null) {
            return undefined;
        }
        const unlistenSelection = listen('selection_translate', () => {
            const selectedText = getSelectedText();
            if (selectedText.trim() !== '') {
                void handleNewText(selectedText);
            }
        });
        return () => {
            unlistenSelection.then((f) => {
                f();
            });
        };
    }, [
