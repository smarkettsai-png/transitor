...
import { ecdictAvailable } from '../../../../../services/translate/ecdict/availability';
import { addAvailableTranslateServices, TATOEBA_PLUGIN_ID } from '../../../../../utils/service_activation';
...
export default function Translate(props) {
    ...
    useEffect(() => {
        ...
        let cancelled = false;
        Promise.all([
            ecdictAvailable().catch(() => false),
            (async () => {
                if (pluginList?.[TATOEBA_PLUGIN_ID]) {
                    return true;
                }
                try {
                    return await exists(await configPath(`plugins/translate/${TATOEBA_PLUGIN_ID}/info.json`));
                } catch {
                    return false;
                }
            })(),
        ]).then(([ecdictAvailableNow, tatoebaAvailable]) => {
            if (cancelled) return;
            const next = addAvailableTranslateServices(translateServiceInstanceList, {
                tatoeba: tatoebaAvailable,
                ecdict: ecdictAvailableNow,
            });
            if (next.length !== translateServiceInstanceList.length) {
                setTranslateServiceInstanceList(next);
            }
        });
        return () => {
            cancelled = true;
        };
    }, [pluginList, translateServiceInstanceList]);
    ...
    const updateServiceInstanceList = (instanceKey) => {
        if (translateServiceInstanceList === null || translateServiceInstanceList.includes(instanceKey)) {
            return;
        } else {
    ...
    const activateInstalledPlugin = (pluginNames) => {
        if (pluginNames.includes(TATOEBA_PLUGIN_ID)) {
            updateServiceInstanceList(TATOEBA_PLUGIN_ID);
        }
    };
    ...
    return (
        <>
            ...
            <SelectPluginModal
                ...
                onPluginInstalled={activateInstalledPlugin}
            />
            <SelectModal
                isOpen={isSelectOpen}
