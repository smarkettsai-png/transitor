...
export default function History() {
    ...
    const init = async () => {
        try {
            const count = await invoke('history_count');
            setTotal(count);
        } catch (error) {
    ...
    const getData = async () => {
        try {
            const result = await invoke('history_list', { page });
            setItems(result);
        } catch (error) {
            toast.error(error.toString(), { style: toastStyle });
        }
    ...
    const getSelectedData = async (id) => {
        try {
            const result = await invoke('history_get', { id });
    ...
    const loadPluginList = async () => {
        ...
        for (const serviceType of serviceTypeList) {
            temp[serviceType] = {};
            if (await exists(await configPath(`plugins/${serviceType}`))) {
                const plugins = await readDir(await configPath(`plugins/${serviceType}`));
                for (const plugin of plugins) {
                    const infoStr = await readTextFile(
                        await configPath(`plugins/${serviceType}/${plugin.name}/info.json`)
                    );
