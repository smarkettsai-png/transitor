...
export default function Recognize() {
    ...
    const loadPluginList = async () => {
        let temp = {};
        if (await exists(await configPath('plugins/recognize'))) {
            const plugins = await readDir(await configPath('plugins/recognize'));
            for (const plugin of plugins) {
                const infoStr = await readTextFile(
                    await configPath(`plugins/recognize/${plugin.name}/info.json`)
                );
