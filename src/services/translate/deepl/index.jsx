...
import { getDeepLOneshotLanguage } from './language';
...
async function translate_by_free(text, from, to) {
    const body = {
        text: [text],
        target_lang: getDeepLOneshotLanguage(to),
    };
    if (from !== 'auto') {
        body.source_lang = getDeepLOneshotLanguage(from);
    }

    const res = await fetch('https://oneshot-free.www.deepl.com/v1/translate', {
        method: 'POST',
        body: Body.json(body),
        headers: {
            Authorization: 'None',
